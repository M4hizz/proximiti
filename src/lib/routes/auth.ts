...
import { AuthService, type AuthenticatedRequest, createRateLimiter } from '../auth';
...
router.post('/auth/google', authRateLimit, async (req: AuthenticatedRequest, res: Response) => {
  try {
...
router.post('/auth/refresh', async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const { accessToken, refreshToken: newRefreshToken } = AuthService.refreshToken(refreshToken);
...
router.post('/auth/logout-all', async (req: AuthenticatedRequest, res: Response) => {
  try {
    if (!req.user) {
      return res.status(401).json({
